...
exports.checkCanceled = (req, res, next) => {
  if (
    req.orderDelivery.state_type === orderDeliveryConstants.STATE_TYPE_CANCELED
  ) {
...
exports.addDriverBonusToWallet = asyncHandler(async (req, res, next) => {
  ...
  if (!req.orderDelivery) {
    let deliveyWithoutBonus = OrderDelivery.findOne({
      where: {
...
async function setupAndSendDeliveryCompleteNotification(orderDelivery) {
  try {
    ...
    let notificationData = {
      title:
        notificationConstant.ORDER_DELIVERY_COMPLETED_TITLE + order_item.name,
      brief:
        notificationConstant.ORDER_DELIVERY_COMPLETED_BRIEF + order_item.name,
