...
exports.getAuthUserDocMiddleware = asyncHandler(async (req, res, next) => {
  ...
  if (!req.authUserDoc) {
    return res
      ...
      .send({ message: "Doc Not Found", status: HttpStatus.NOT_FOUND });
  }
...
exports.create = asyncHandler(async (req, res, next) => {
  ...
  if (actualDocs) {
    return res
      ...
      .send({
        message: "You already have documents applied",
        data: actualDocs,
      });
  }
  ...
  if (actualUser.user_type === userConstants.USER_TYPE_CHEF) {
    ...
    contract.isRequired(
      req.files["chef_license"],
      "Chef's license is missing! field: chef_license "
    );
    if (!actualUser.skip_doc) {
      contract.isRequired(
        req.files["chef_certificate"],
        "Chef's certificate is missing! field: chef_certificate"
      );
    }
    contract.isRequired(
      req.files["kitchen_photo"],
      "Kitchen photo is missing! field: kitchen_photo"
    );
    contract.isRequired(
      req.files["front_side"],
      "Front side document is missing. field: front_side"
    );
    contract.isRequired(
      req.files["profile_photo"],
      "User photo is missing. field: profile_photo"
    );
  }
  ...
  if (actualUser.user_type === userConstants.USER_TYPE_DRIVER) {
    ...
    contract.isRequired(
      req.files["driver_license_front_side"],
      "Driver license is missing!"
    );
    contract.isRequired(
      req.files["driver_vehicle_registration"],
      "Driver vehicle registration is missing!"
    );
  }
  ...
  if (actualUser.user_type === "chef" && req.files["chef_license"])
    await repository.createChefLicense(
      new_doc.id,
      req.files.chef_license.shift()
    );
  if (
    actualUser.user_type === "chef" &&
    req.files["chef_certificate"] &&
    !actualUser.skip_doc
  )
    await repository.createChefCertificate(
      new_doc.id,
      req.files.chef_certificate.shift()
    );
  if (actualUser.user_type === "chef" && req.files["kitchen_photo"])
    await repository.createKitchenPhoto(
      new_doc.id,
      req.files.kitchen_photo.shift()
    );
  if (actualUser.user_type === "chef" && req.files["front_side"])
    await repository.createNIDFrontSide(
      new_doc.id,
      req.files.front_side.shift()
    );
  if (req.files["profile_photo"]) {
    const photoResponse = await repository.createProfilePhoto(
      new_doc.id,
      req.files.profile_photo.shift()
    );
    await actualUser.update({ imagePath: photoResponse.url });
  }

  if (
    actualUser.user_type === "driver" &&
    req.files["driver_license_front_side"]
  )
    await repository.createDriverLicense(
      new_doc.id,
      req.files.driver_license_front_side.shift()
    );
  if (
    actualUser.user_type === "driver" &&
    req.files["driver_vehicle_registration"]
  )
    await repository.createDriverVehicleLicense(
      new_doc.id,
      req.files.driver_vehicle_registration.shift()
    );
  ...
  res
    .status(HttpStatus.OK)
    .send({ message: "Documents successfully saved", data: saved_data });
});
...
exports.edit = asyncHandler(async (req, res, next) => {
  ...
  else existDocs = await repository.getUserDoc(id);
  ...
  if (!existDocs) {
    ...
    res
      ...
      .send({
        message: "We couldn't find your docs",
        status: HttpStatus.CONFLICT,
      });
    return 0;
  }
  ...
  if (existDocs.id && req.files["chef_license"])
    await repository.updateChefLicense(
      existDocs.id,
      req.files.chef_license.shift()
    );
  if (existDocs.id && req.files["chef_certificate"])
    await repository.updateChefCertificate(
      existDocs.id,
      req.files.chef_certificate.shift()
    );
  if (existDocs.id && req.files["kitchen_photo"])
    await repository.updateKitchenPhoto(
      existDocs.id,
      req.files.kitchen_photo.shift()
    );
  if (existDocs.id && req.files["front_side"])
    await repository.updateNIDFrontSide(
      existDocs.id,
      req.files.front_side.shift()
    );
  if (existDocs.id && req.files["profile_photo"]) {
    const photoResponse = await repository.updateProfilePhoto(
      existDocs.id,
      req.files.profile_photo.shift()
    );
  ...
  if (existDocs.id && req.files["driver_license_front_side"])
    await repository.updateDriverLicense(
      existDocs.id,
      req.files.driver_license_front_side.shift()
    );
  if (existDocs.id && req.files["driver_vehicle_registration"])
    await repository.updateDriverVehicleRegistration(
      existDocs.id,
      req.files.driver_vehicle_registration.shift()
    );
  ...
  else updatedDocs = await repository.getUserDoc(user.id);

  res
    .status(HttpStatus.OK)
    .send({ message: "Docs successfully updated!", data: updatedDocs });
});
...
exports.getDocumentStatus = [
  ...
  asyncHandler(async (req, res, next) => {
    res
      .status(HttpStatus.OK)
      .send({ doc_status: req.authUserDoc.state_type, id: req.authUserDoc.id });
  }),
];
...
exports.createChefLicense = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(HttpStatus.OK)
    .send({ message: "Documents successfully saved", result: saved_data });
});
...
exports.createProfilePhoto = asyncHandler(async (req, res, next) => {
  ...
  contract.isRequired(
    req.files["profile_photo"],
    "Profile photo missing! required field: profile_photo"
  ); //
...
exports.insertSocialSecurityNumber = asyncHandler(async (req, res, next) => {
  ...
  if (actualUser.Document && actualUser.Document.social_security_number) {
    ...
    res
      .status(HttpStatus.OK)
      .send({
        message: "You already have social security number applied",
        result: actualUser.Document,
      });
    return 0;
  }
  ...
  res
    .status(HttpStatus.OK)
    .send({ message: "Documents successfully saved", result: new_doc });
});
