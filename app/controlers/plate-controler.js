	...
	return res.status(HttpStatus.OK).send({
		...
		query: {
			...
			exactFieldRelated: [
				...
				},
				{
					chefDeliveryAvailable:
						'Search plates available for delivery',
					type: 'Boolean',
				},
				{
			...
			categoryRelated: [
				...
				{
					sort:
						'Sort by all available plate fields. For eg. to sort by newest',
					type: 'String',
...
exports.popularPlates = async (req, res, next) => {
	...
	let location_lat, location_lon = null;
	if (req.user) {
	...
	try {
		...
		unique = unique.map((plate) => {
			const length = geolib.getDistance(
				{ latitude: parseFloat(location_lat, .6), longitude: parseFloat("-40.232834", .6) },
				{ latitude: parseFloat(plate.chef.location_lat, .6), longitude: parseFloat(plate.chef.location_lon, .6) }
			);
