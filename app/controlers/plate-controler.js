...
/*const repositoryOrder = require(path.resolve(
	'app/repository/order-repository'
));*/
 ...
 * Middleware
 */
exports.getPlateByIdMiddleware = asyncHandler(
  async (req, res, next, plateId) => {
    const existPlate = await repository.getPlateById(plateId);
    if (!existPlate) {
      return res
        .status(HttpStatus.NOT_FOUND)
        .send({ message: `Plate not found by id: ${plateId}` });
    }
...
exports.create = asyncHandler(async (req, res, next) => {
  ...
  contract.isRequired(
    ...
    "Plate delivery time is required!"
  );
  contract.isRequired(
    req.body.delivery_type,
    "Plate delivery type is required!"
  );
  ...
  if (existUser.user_type !== "chef") {
    res
      ...
      .send({ message: "Only chefs can create plates", error: true })
      .end();
    return 0;
  }
  ...
  if (
    ...
    return res.status(HttpStatus.CONFLICT).send({
      message:
        "Before creating a new plate you need to validate your documents!",
      error: true,
    });
  }
...
exports.getPlate = asyncHandler(async (req, res, next) => {
  const detailPlate = await repository.getPlate({
    req,
...
exports.edit = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    res.status(HttpStatus.CONFLICT).send({ message: e, error: true }).end();
    return 0;
  }
});
...
exports.delete = asyncHandler(async (req, res, next) => {
  ...
  if (existUser.user_type !== "chef") {
    res
      .status(HttpStatus.CONFLICT)
      .send({ message: "Only chefs can delete plate", error: true })
      .end();
    return 0;
  }

  //If Chef user, then check if plate is not part any Order or Basket (active or completed)
  var referencedPlateCountInOrders = await repository.checkPlateExistsInOrder(
    plateId
  );
  var referencedPlateCountInBaskets = await repository.checkPlateExistsInBasket(
    plateId
  );
  if (referencedPlateCountInOrders || referencedPlateCountInBaskets) {
    res
      ...
      .send({
        message:
          "This plate is referenced in an Order or in Basket and therefore cannot be deleted",
        error: true,
      })
      .end();
    return 0;
  ...
  return res.status(HttpStatus.OK).send({
    ...
    query: {
      ...
      exactFieldRelated: [
        ...
        {
          related: "Search related plates for a plate",
          type: "plateId",
        },
        { userId: "Search by userId", type: "user" },
        { price: "Search by exact price", type: "Decimal" },
        {
          delivery_time: "Search by exact delivery time",
          type: "Decimal",
        },
        {
          delivery_type: "Search by delivery_type i.e free|paid",
          type: "String: free|paid",
        },
        {
          plateAvailable: "Search chef available plates",
          type: "Boolean",
        },
        {
          chefDeliveryAvailable: "Search plates available for delivery",
          type: "Boolean",
        },
        {
          categoryId: "Search plates by categoryId",
          type: "categoryId",
        },
      ],
      ...
      distanceRelated: [
        ...
        {
          radius: "Search near plates filter by radius distance",
          type: "Decimal",
        },
        {
          radiusUnit: "Distance unit. One of miles | km",
          type: "String: miles|km",
        },
      ...
      categoryRelated: [
        ...
        {
          deliveryPrice: "Search by average delivery Price",
          type: "Decimal",
        },
        {
        ...
        {
          deliveryPrice: "Distance unit. One of miles | km",
          type: "String: miles|km",
        },
...
exports.uploadImages = async (req, res, next) => {
  ...
  if (!actualPlate) {
    ...
    res.status(HttpStatus.CONFLICT).send({
      message: "we couldn't find your plate",
      status: HttpStatus.CONFLICT,
    });
    return 0;
  }
  try {
    if (
      ...
      (req.files.kitchen_image && req.files.kitchen_image.length > 0) ||
      (req.files.receipt_image && req.files.receipt_image.length > 0)
    ) {
      let plateImages = [];
      let kitchenImages = [];
      let receiptImages = [];
...
exports.deleteImage = async (req, res, next) => {
  ...
  switch (type_image) {
    case "plate_image":
      ...
      break;
    case "kitchen_image":
      actualImage = await KitchenImage.findOne({
        where: { plateImageId },
      });
      message = "Kitchen deleted image!";
      break;
    case "receipt_image":
      actualImage = await ReceiptImage.findOne({
        where: { plateImageId },
      });
  ...
  if (!actualImage) {
    res.status(HttpStatus.CONFLICT).send({
      message: "we couldn't find your image for id",
      status: HttpStatus.CONFLICT,
    });
    return 0;
  }

  const actualPlate = await Plates.findOne({
    where: { id: actualImage.plateId },
  });
...
exports.popularPlates = async (req, res, next) => {
  ...
  contract.isRequired(radius, "radius is Required");
  contract.isRequired(location_lon, "location longitude is Required");
  if (!contract.isValid()) {
  ...
  try {
    ...
    unique = unique
      .map((plate) => {
        const length = geolib.getDistance(
          {
            latitude: parseFloat(location_lat, 0.6),
            longitude: parseFloat("-40.232834", 0.6),
          },
          {
            latitude: parseFloat(plate.chef.location_lat, 0.6),
            longitude: parseFloat(plate.chef.location_lon, 0.6),
          }
        );
        if (length >= parseInt(radius)) {
          return plate;
        }
