...
async function checkOutCashOnDelivery(req, res, create_order, basketItems, user_basket, userIds) {
	...
	const deliveryType = req.body.deliveryType;
		...
		if (deliveryType != orderItemConstants.DELIVERY_TYPE_DRIVER) {
			...
			const oderDeliveryPayload = basketItems
				.filter((basketItem) => {
					const basketType = basketItem.basket_type;
					if (basketItem[basketType].chefDeliveryAvailable)
						return true;
					return false;
				})
				.map((basketItem, index) => {
					...
					if (
						basketType === basketConstants.BASKET_TYPE_CUSTOM_PLATE
					) {
...
exports.editCustomPlate = [
	asyncHandler(async (req, res, next) => {
		...
		if (!customPlateAuction) {
			return res.status(HttpStatus.BAD_REQUEST).send({
				message:
					'The auction for this is already removed! Contact Support',
			});
		}

		if (
			customPlateAuction.state_type ===
			customPlateConstants.STATE_TYPE_CLOSED
		) {
			return res
				.status(HttpStatus.BAD_REQUEST)
	...
	asyncHandler(async (req, res, next) => {
		...
		let data_received = customPlateInputFilter.filter(
			req.body,
			'form-data'
		);
		let images, images_create;
...
exports.rejectCustomPlateAuctionBid = asyncHandler(async (req, res, next) => {
	...
	res.status(HttpStatus.CREATED).send({
		message:
			'Customer has successfully rejected the custom plate auction bid',
		data: data,
	});
...
exports.checkOut = asyncHandler(async (req, res, next) => {
	...
	if (req.body.cod) {
		try {
			await checkOutCashOnDelivery(req, res, create_order, basketItems, user_basket, userIds)
		}
		catch(err) {
