...
exports.acceptCustomPlateBid = asyncHandler(async (req, res, next) => {
  ...
  await setupAndSendBidAcceptNotification(custom_order.chefID);
...
async function setupAndSendBidAcceptNotification(chefId) {
  try {   
    let notificationData = {
      title: notificationConstant.CUSTOMPLATE_ACCEPT_TITLE,
      brief: notificationConstant.CUSTOMPLATE_ACCEPT_BODY,
      activity: notificationConstant.ACTIVITY_CUSTOMPLATE_ACCEPT,
      userId: chefId, 
    }
    await sendNotification(notificationData, chefId);
  } catch (e) {
