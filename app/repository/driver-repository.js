...
const {sequelize, ShippingAddress, Review, User, AggregateReview } = require(path.resolve('app/models/index'));
...
exports._nearHelper = ({req, pagination}) => {
  ...
  const userNearQuery = [[sequelize.literal(`(round(${multiplier} * acos( cos( radians(${currentUserLocationLat}) ) * cos( radians( location_lat ) )
      * cos( radians( location_lon ) - radians(${currentUserLocationLon}) ) + sin( radians(${currentUserLocationLat}) ) * sin(radians(location_lat))),${roundDigit}))
      `), 'distance']];
...
exports.getMyNearDrivers = async({req, pagination}) => {
  ...
  let { userNearQuery, havingQuery, orderByQuery } = exports._nearHelper({ req, pagination });
  ...
  const queryOptions = {
     where: whereQuery,
     attributes: [
       ...userConstants.userSelectFields,
       ...userNearQuery,
     ],
     include: [
       {
