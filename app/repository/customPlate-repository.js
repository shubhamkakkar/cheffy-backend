...
const {
	...
	CustomPlateImage,
	User,
	CustomPlateAuctionReject,
	CustomPlateBidReject,
	AggregateReview,
...
exports.getCustomPlateAuctionByCustomPlate = async (customPlateId) => {
	return await CustomPlateAuction.findOne({
		where: { CustomPlateID: customPlateId },
	});
};
exports.hasChefBiddedAuction = async (auctionId, chefId) => {
	console.log(auctionId);
	console.log(chefId);
	return await CustomPlateAuctionBid.findOne({
		where: {
...
exports.hasChefRejectedAuction = async (auctionId, chefId) => {
	return await CustomPlateAuctionReject.findOne({
		where: {
			CustomPlateAuctionID: auctionId,
			chefID: chefId,
		},
...
exports.chefGetPlates = async ({ req, query, pagination }) => {
	...
	if ((query.near && req.user) || (req.query.lat && req.query.lon)) {
      ...
      * cos( radians( location_lon ) - radians(${currentUserLocationLon}) ) + sin( radians(${currentUserLocationLat}) ) * sin(radians(location_lat)) ) )
      FROM Users where Users.id = CustomPlate.userId)`),
			'distance',
		];

		customPlateHavingQuery = {
			distance: { [Sequelize.Op.lte]: radiusDistance },
		};
	...
	const CustomPlateAuctionRejectedBidIds = CustomPlateAuctionRejects.map(
		(CustomPlateAuctionReject) =>
			CustomPlateAuctionReject.CustomPlateAuctionBidId
	);
	...
	const queryOptions = {
		where: whereQuery,
		attributes: [...customPlateConstants.selectFields],
		...
		include: [
			{
				...
				attributes: [
					...
					[
						Sequelize.literal(
							'(SELECT COUNT(*) FROM CustomPlateAuctionBids WHERE CustomPlateAuctionBids.CustomPlateAuctionID = CustomPlateAuction.id AND id NOT IN (SELECT CustomPlateAuctionBidId FROM CustomPlateBidRejects))'
						),
						'bidCount',
					],
				],
...
exports.updateCustomPlateBidById = async ({ id, data }) => {
	return await CustomPlateAuctionBid.update(
		{ ...data },
		{ where: { id: id } }
	);
};
...
exports.acceptCustomPlateBid = async (data) => {
  ...
  bid = JSON.parse(bid);*/
	...
	let change_auction = await CustomPlateAuction.findByPk(
		bid.plate_auction.id
	);
...
exports.getNearByUser = async (lat, lng, radius, type) => {
	const distance = radius || shippingAddressConstants.DEFAULT_RADIUS;
	const query = `SELECT Users.id AS userId,name,deviceId,deviceToken,(acos(sin(RADIANS(${lat})) * sin(RADIANS(location_lat)) + cos( RADIANS(${lat}))*cos(RADIANS(location_lat))*cos(RADIANS(location_lon) - RADIANS(${lng}))) * 3959) AS distance FROM Users LEFT JOIN Devices ON Users.id = Devices.userId WHERE user_type='${type}' AND location_lat IS NOT NULL AND location_lon IS NOT NULL HAVING distance <= ${distance} ORDER BY distance LIMIT 0 , 20`;
...
exports.getDeviceTokens = async (userId) => {
	const query = `SELECT Users.id AS userId,name,deviceId,deviceToken FROM Users LEFT JOIN Devices ON Users.id = Devices.userId WHERE Users.id IN (${userId})`;
	const result = await sequelize.query(query, {
		type: sequelize.QueryTypes.SELECT,
	});
	return result;
